...
void LCD_1IN28_Clear(UWORD Color)
{
	...
	{
		uint16_t line_data[LCD_1IN28_WIDTH];
		memset(line_data, 0x0, sizeof(line_data));
		lcd_spi_write_pack(line_data, LCD_1IN28_WIDTH);
		DEV_Delay_ms(2);
	}
}
...
void LCD_1IN28_Display(UWORD *Image)
{
	...
	for (i = 0; i < LCD_1IN28_WIDTH; i++)
	{
		...
		{
			LCD_1IN28_SendData_16Bit(*(Image + i * LCD_1IN28_WIDTH + j));
		}
	}
}
...
void LCD_1IN28_DisplayWindows(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend, UWORD *Image)
{
	...
	//查看颜色值
	// printf("%x\n", *Image);
	lcd_spi_write_pack(Image, totalpoint);
}
